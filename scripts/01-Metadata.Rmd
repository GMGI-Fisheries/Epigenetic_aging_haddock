...
fall2023 <- read_excel("data/00_metadata/age data/Fall2023_haddock_finclips_aged.xlsx") %>%
  ...
  mutate(
    ...
    `Organism Id`=as.character(`Organism Id`)
  )
...
```{r}
data <- read_xlsx("data/00_metadata/full_finclips_sampling.xlsx")
sequencing <- read_xlsx("C:/BoxDrive/Box/Science/Fisheries/Projects/Epigenetic Aging/_Haddock/Labwork/Haddock_labwork.xlsx", 
                        sheet = "Sample List") %>% dplyr::select(GMGI_ID, `Seq Rnd`) %>% left_join(., data, by = "GMGI_ID")

sequencing %>% write_xlsx("data/00_metadata/sequencing_metadata.xlsx")
```
...
```{r}
data_demographics <- data %>% mutate(AgeRounded = as.factor(AgeRounded)) %>% dplyr::select(-Sex) %>%
...
1. Length w/ age, sex, season   
2. Weight w /age, sex, season

test whether length differs by sex while accounting for age

```{r}
library(mgcv)

stats_df <- data %>% filter(!is.na(`Ind Sex`)) %>% filter(!is.na(`Length Cm`)) %>% filter(!is.na(`Weight Kg`)) %>% ##421
  dplyr::select(Trawl_ID, Length=`Length Cm`, Weight=`Weight Kg`, Sex=`Ind Sex`, AgeRounded, 
                Latitude=`Latitude Dec`, Longitude=`Longitude Dec`, AgeRounded)

## s(variable) allows that variable to have a non-linear curve
## REML = restricted maximum likelihood - determine how smooth the curve should be / default choice 
length_mod <- gam(Length ~ Sex + s(AgeRounded), data = stats_df, method = "REML")
summary(length_mod)

weight_mod <- gam(Weight ~ Sex + s(AgeRounded), data = stats_df, method = "REML")
summary(weight_mod)
```
...
```{r}
sequencing_stats <- sequencing %>% dplyr::rename(Lat=`Latitude Dec`, Long=`Longitude Dec`)

gam_age <- gam(
  AgeRounded ~ s(Lat, Long),
  data = sequencing_stats,
  method = "REML"
)
...
sequencing %>% 
  ...
  geom_point(
    data = sequencing %>% select(-AgeRounded),
    color = "grey80",
    size = 1
  ) +
  
  geom_point(aes(fill = AgeRounded),
             shape = 21, size = 2) +
  
  facet_wrap(~ AgeRounded) +
  theme_bw()

ggsave("data/02_catch_data/age_by_location.png", width=9, height=6.5)
```
