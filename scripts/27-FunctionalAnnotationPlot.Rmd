...
```{r}
functional_clock <- read.csv("data/06_genomic_function/functional_clock_8-7-2026.csv") %>% dplyr::select(-X)
length(unique(functional_clock$Loc)) ##81; 8-7-2026 86 

# folder is zipped 
# functional_100p <- read.csv("data/06_genomic_function/functional_100p.csv") %>% dplyr::select(-X)
# length(unique(functional_100p$Loc))

functional_90p <- read.csv("data/06_genomic_function/functional_90p_8-7-2026.csv") %>% dplyr::select(-X)
length(unique(functional_90p$Loc)) # 33,703; 8-7-2026 = 49,962 
```

```{r}
# 4. Read in prediction data for the Ages of individuals used
best_clock <- read.csv("data/04_age_prediction/predictions-8-5-2026.csv") %>% dplyr::select(GMGI_ID, AgeRounded) %>% arrange(AgeRounded)
sample_order <- best_clock$GMGI_ID
age_order <- best_clock$AgeRounded

# 1. Read data
clock_data <- read.csv("data/04_age_prediction/clock_data-8-7-2026.csv") %>% dplyr::select(-X)
full100_data <- read.csv("data/04_age_prediction/full100p_data-8-7-2026.csv") %>% dplyr::select(-X)
full90_data <- read.csv("data/04_age_prediction/full90p_data-8-7-2026.csv") %>% dplyr::select(-X)
...
full90_data %>% filter(is.na(percent.meth))
...
z_col_fun <- colorRamp2(c(-2, 0, 2), c("grey", "white", "#013a63"))

pdf(
  "manuscript figures/Figure4_heatmap_90p_v2.pdf",
  width = 9,
  height = 9
)
...
cluster1_names <- as.data.frame(rownames(clock_matrix_z)[cluster1_rows]) %>% rename(Loc=1) %>% 
  mutate(cluster="1") %>% ## 30 loci
  mutate(Loc = str_after_nth(Loc, "\\|", 2))

cluster2_names <- as.data.frame(rownames(clock_matrix_z)[cluster2_rows]) %>% rename(Loc=1) %>% 
  mutate(cluster="2") %>% ## 79 loci
  mutate(Loc = str_after_nth(Loc, "\\|", 2))
...
cluster_information %>% group_by(cluster, Note) %>% summarise(count = n_distinct(Loc)) %>% write_xlsx("data/06_genomic_function/functional_notes.xlsx")

length(unique(cluster_information$Name))

### Export gene list 
cluster_information %>% dplyr::select(Name, cluster) %>% distinct() %>% arrange(Name) %>% filter(!is.na(Name)) %>%
  mutate(Name = gsub("Name=", "", Name)) %>% write.csv("data/06_genomic_function/cluster_genelist.csv", row.names = FALSE)
```
...
cluster1_full_names <- as.data.frame(rownames(which_matrix)[cluster1_full_rows]) %>% 
  rename(Loc=1) %>% mutate(cluster="1") ## 3,554 loci for 100% ; 28,762 loci for 90%; 26,239 loci for 90% 8-10-2026

cluster2_full_names <- as.data.frame(rownames(which_matrix)[cluster2_full_rows]) %>% 
  rename(Loc=1) %>% mutate(cluster="2") ## 4,177 loci; 18,611 loci for 90%; 41,602 loci for 90% 8-10-2026 
length(unique(cluster2_full_names$Loc))
...
cluster_full_information %>% group_by(cluster) %>%
  #summarise(count = n_distinct(Name)) %>% 
  write_xlsx("data/06_genomic_function/functional_90p.xlsx")
...
```{r}
### confirming hypomethylation
full90_data %>% subset(Loc == "ENA_OLKM01001373_OLKM01001373.1_87693") %>%
  left_join(., best_clock %>% dplyr::rename(sampleID=GMGI_ID), by = "sampleID") %>%
  
  ggplot(., aes(x=AgeRounded, y=percent.meth)) + 
  geom_point() +
  theme_bw()
```
