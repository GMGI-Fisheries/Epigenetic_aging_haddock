...
```{r}
t.test(data$`Aligned (%)` ~ data$`Seq Rnd`)
t.test(data$`Conversion Efficiency (%)` ~ data$`Seq Rnd`)

t.test(WGBS_data$`mCpG (%)` ~ data$`Seq Rnd`)
t.test(WGBS_data$`mCHG (%)` ~ data$`Seq Rnd`)
t.test(WGBS_data$`mCHH (%)` ~ data$`Seq Rnd`)
t.test(WGBS_data$`Cytosines (M)` ~ data$`Seq Rnd`)
t.test(WGBS_data$`Aligned (%)` ~ data$`Seq Rnd`)
t.test(WGBS_data$`Duplicates (%)` ~ data$`Seq Rnd`)
```
...
### Coverage vs. 10x per site filter 

```{r}
depth <- read.delim2("data/01_QC/10X_sample_depth2.txt", sep = " ", header=F) %>% 
  dplyr::select(CpGs=V5, path=V6) %>%
   mutate(sample = str_before_first(path, "_S"),
          sample = str_after_first(sample, "files/"),
          CpGs = as.numeric(CpGs))

total_depth <- read_xlsx("data/01_QC/sequencing statistics.xlsx") %>%
  mutate(coverage = (`M Seqs`*2*10^6*150)/(652.8*10^6) ) %>%
  arrange(desc(coverage)) %>%
  mutate(sample = str_before_first(`Sample Name`, "_"))

depth_df <- depth %>% dplyr::select(-path) %>% full_join(., total_depth %>% dplyr::select(sample, coverage))

depth_df %>%
  ggplot(., aes(x=coverage, y=CpGs)) + 
  geom_point() + 
  theme_bw()
```
