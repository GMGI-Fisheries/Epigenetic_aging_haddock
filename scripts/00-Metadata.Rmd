...
```{r}
demo_data <- manuscript_data %>% dplyr::rename(Sex=`Ind Sex`, Weight=`Weight Kg`, Length=`Length Cm`) %>%
  mutate(AgeRounded = as.factor(AgeRounded))
colnames(demo_data)

range(demo_data$Weight, na.rm = TRUE)
...
summarySE(
  ...
  filter(!is.na(AgeRounded)) %>% write_xlsx("data/02_catch_data/Manuscript_weight_data.xlsx")

summarySE(
  data = demo_data,
  groupvars = c("Sex", "AgeRounded"),
  measurevar = "Length"
) %>% dplyr::select(-sd, -ci) %>% dplyr::rename(mean=Length) %>%
  filter(!is.na(AgeRounded)) %>% write_xlsx("data/02_catch_data/Manuscript_length_data.xlsx")
```
