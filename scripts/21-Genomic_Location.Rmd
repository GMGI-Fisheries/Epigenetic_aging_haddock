...
#clock_data %>% write.csv("/projects/gmgi/Fisheries/epiage/haddock/clock_data.csv")
```

gff information

```{r}
assembly_report <- read.delim2("/projects/gmgi/Fisheries/reference_genomes/Haddock/GCA_900291075.1_MAEA_1_assembly_report.txt", skip = 25) %>% 
...
## pick which df to use
meth_df <- methylation90p_data
```

```{r}
# Create GRanges objects
meth_gr <- GRanges(
  seqnames = meth_df$scaffold,
  ranges = IRanges(start = meth_df$position, width = 1)
)
################
...
overlap_df <- data.frame(
  meth_df[queryHits(overlaps), ],  # Methylation info
...
##### couldn't get the right information in script 25 so doing here for now 
overlap_df_genes <- overlap_df %>% filter(!grepl("match|contig|mRNA", feature_type)) %>% filter(grepl("gene", feature_type)) %>%
  separate(V9, c("ID", "Name", "Alias", "Note"), sep = ";") %>%
  mutate(Note = gsub("Note=", "", Note),
         Name = gsub("Name=", "", Name))
  
length(unique(overlap_df_genes$Loc)) ## 81 90% = 33,703

overlap_df_genes %>% write.csv("/projects/gmgi/Fisheries/epiage/haddock/functional_90p.csv")
```

```{r}
overlap_df %>% dplyr::select(Loc, scaffold, position, Function=V9) %>%
  filter(!grepl("Parent=", Function)) %>%
  mutate(
    Function = str_after_nth(Function, ";", 1)
  ) %>% distinct() %>% write.csv("/projects/gmgi/Fisheries/epiage/haddock/90p_genomicfunction.csv")
...
promoter_overlaps_df <- data.frame(
  meth_df[queryHits(promoter_overlaps), ],  # Methylation info
...
terminator_overlaps_df <- data.frame(
  meth_df[queryHits(terminator_overlaps), ],  # Methylation info
...
```{r}
export_df <- meth_df %>% dplyr::select(Loc) %>% distinct() %>% full_join(

rbind(three_prime_UTR, five_prime_UTR, CDS) %>% dplyr::select(Loc, feature_type) %>% distinct() %>% 
  rbind(
    overlap_df %>% dplyr::select(Loc) %>% distinct() %>% mutate(feature_type="Intron"),
    promoter_overlaps_df %>% dplyr::select(Loc) %>% distinct() %>% mutate(feature_type="Promoter"),
    terminator_overlaps_df %>% dplyr::select(Loc) %>% distinct() %>% mutate(feature_type="Terminator")
  )
  ) %>% 

  mutate(feature_type = ifelse(is.na(feature_type), "Other", feature_type)) %>% 
  distinct() %>%
  
  dplyr::group_by(Loc) %>%
  dplyr::filter(
    # Keep all rows if Loc doesn't have both promoter and terminator
    !(any(feature_type == "Promoter") & any(feature_type == "Terminator")) |
    # For Locs with both, keep rows that are NOT terminator (i.e. keep promoter)
    feature_type != "Terminator"
  ) %>%
  ungroup() %>%
  
  mutate(
    feature_type = gsub("three_prime_UTR", "UTR3", feature_type),
    feature_type = gsub("five_prime_UTR", "UTR5", feature_type)
  )

export_df %>% write.csv("/projects/gmgi/Fisheries/epiage/haddock/90p_genomiclocation.csv")
```
...
# Other_count <- length(unique(meth_df$Loc)) - Gene_count - length(unique(promoter_overlaps_df$Loc)) - length(unique(terminator_overlaps_df$Loc))
#   
...
figure_data <- export_df %>% dplyr::group_by(feature_type) %>%
  summarise( Value = n_distinct(Loc),
              .groups = "drop"
  )

figure_data %>% write.csv("/projects/gmgi/Fisheries/epiage/haddock/90p_genomiclocation_summary.csv")
#figure_data %>% write.csv("/work/gmgi/Fisheries/epiage/haddock/90p_genomiclocation.csv")
...
ggsave("/projects/gmgi/Fisheries/epiage/haddock/Genomic_locations_90p.png", width=4, height=5)
```
...
scaffold_loci_90p %>% write.csv("/projects/gmgi/Fisheries/epiage/haddock/scaffold_loci_length_90p.csv", row.names = FALSE)
```
