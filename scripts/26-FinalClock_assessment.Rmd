...
```{r}
best_clock <- read.csv("data/04_age_prediction/predictions-8-5-2026.csv")
...
best_clock %>% 
  ...
  write_xlsx("data/04_age_prediction/Haddock_age_prediction-8-7-2026.xlsx")
```
...
ggsave("manuscript figures/Figure2_MAE.png", width=3.5, height=5)
...
```{r}
mae %>% ggplot(., aes(x=AgeRounded, y=mae)) + 
  geom_boxplot(fill=NA, aes(color=group, group=AgeRounded), width=0.4, outlier.shape=NA) +
  geom_point(aes(fill=group), width=0.05, color='black', shape=21, size=1.5, alpha=0.75) +
  facet_wrap(~group) +
  scale_fill_manual(values = c("#588157", "grey90")) +
  scale_color_manual(values = c("#588157", "grey70")) +
  labs(
    y = "Mean Absolute Error (years)",
    x = "Age (years)"
  ) +
...
ggsave("manuscript figures/Supplemental_Figure6_v2.png", width=8, height=3.5)
```
